...
Compiles the standalone `pboss` executable and sets up the per-user boot service. No root required: the binary goes to `~/.local/bin` (Linux/macOS — the installer adds it to your `PATH` automatically when missing) or `%LOCALAPPDATA%\pboss` (Windows).
...
If the command is not found, make sure the install directory is on your `PATH`: `~/.local/bin` (one-line installer — added to your shell profile automatically, open a new terminal), `~/.bun/bin` (Bun global), or `/usr/local/bin` (root install).
