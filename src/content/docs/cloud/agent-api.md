...
- Deployments — `process.gitinfo`, `deploy.run` (one batch×target job: strategy `release` for created processes — the code lands in `~/apps/{process}/` next to the agent's pboss home, i.e. `/home/{username}/apps` for the OS user running the agent — `inplace` for adopted ones; backups under `~/.pboss/backups/{targetId}/v{n}_{commit}/`), `deploy.restore` (revert to a recorded backup — the same path the failed-health auto-rollback rides), `deploy.cancel`, `deploy.purge` (Remove with "also delete stored backups and files")
