...
**Baseline anomaly** — "using far more than it usually does" — runs for every agent, because "usual" is what a single box can't learn about itself. The trailing-6h median CPU and RSS is what it's *meant* to use; when the recent average clears both a multiple and an absolute step (CPU: 3× and +20 points, floored at 30%; memory: 2.5× and +150 MB) with 20+ samples, one warning fires — *"api-worker CPU at 62% — 4.1× its usual 15%"*. A quietly doubled footprint after a deploy gets noticed without hitting any absolute level; an already-busy one doesn't nag.
